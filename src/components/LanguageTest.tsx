...
import { addCorrect, addError } from "../features/game/languageSlice";
import { decrementHint } from "../features/game/gameSlice";

export function LanguageTest() {
  const correct = useAppSelector((state) => state.language.correct);
  const incorrect = useAppSelector((state) => state.language.incorrect);
  ...
  let regionHint;
  if (isSuccess) {
    language = Object.values(countries[random].languages)[0];
    ...
    >
      ...
      >
        <h1 style={{ textAlign: "center" }}>
          {language} is the offical language of which country?
        </h1>
        <Button
          variant="outlined"
